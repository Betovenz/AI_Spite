...
async function mintFlowCaptcha(action, projectId, signal) {
  const result = await ext.command(ACTION.FLOW_MINT_CAPTCHA, { action, projectId }, 90_000, signal);
  if (!result?.token) throw new Error("Flow ไม่คืน reCAPTCHA token");
  return result.token;
}

async function refreshFlowCaptcha(projectId, signal) {
  await ext.command(ACTION.FLOW_REFRESH_CAPTCHA, { projectId }, 90_000, signal);
}
