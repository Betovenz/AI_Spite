...
function waitForComplete(tabId, timeoutMs = 45_000) {
...
function flowProjectUrl(projectId) {
  ...
  return FLOW_PROJECT_ID.test(value) ? `https://flow.google.com/project/${value}` : "";
}

async function ensureFlowProjectTab(projectId = "") {
  const wanted = flowProjectUrl(projectId);
  const tab = await ensureTab(FLOW_PATTERNS, wanted || FLOW_URL);
  if (wanted && !String(tab.url || "").startsWith("https://flow.google.com/project/")) {
    await chrome.tabs.update(tab.id, { url: wanted });
    await waitForComplete(tab.id);
...
async function mintRecaptchaInPage(siteKey, captchaAction) {
  try {
    ...
    const token = await Promise.race([
      grecaptcha.enterprise.execute(siteKey, { action: captchaAction }),
...
async function dispatch(command) {
  ...
  switch (action) {
    ...
    case ACTION.FLOW_HARVEST_COOKIES: {
      ...
      const tab = await ensureTab(["https://labs.google/*"], LABS_CONNECT_URL);
      let cookieHeader = "";
